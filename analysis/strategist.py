...
class Strategist:
    ...
    def plan_next(self, *, graphs_summary: str, completed: list[str], n: int = 5, 
        ...
        system = (
            ...
            "DIVERSITY-FIRST POLICY (Early → Mid → Late):\n"
            "- EARLY PHASE (e.g., planning iteration ≤ 2 OR nodes coverage < ~30%):\n"
            "  • Maximize BREADTH: ≥70% of items should be broad ASPECT frames across different graphs/subsystems.\n"
            "  • ALWAYS include ≥1 coverage sweep targeting UNVISITED nodes (from COVERAGE STATUS).\n"
            "  • DIVERSITY CONSTRAINTS per batch: (a) ≤1 item per primary contract/module, (b) ≤2 items sharing the same aspect label (e.g., ExternalDependencyTrust), (c) spread items across distinct graphs.\n"
            "    (Primary contract/module = first component in focus_areas, e.g., Function:Whitelist.setPool → Whitelist.)\n"
            "  • ITEM GRANULARITY: Prefer BROAD, high‑level aspects (architecture/flows/roles/invariants). Avoid deep dives unless there is a very strong signal.\n"
            "- MID PHASE (e.g., ~30–70% nodes coverage or planning iteration ~3–5):\n"
            "  • Maintain balance: ~50% broad ASPECTS, ~50% EVIDENCE-BASED SUSPICIONS linked to prior signals.\n"
            "  • Keep ≥1 coverage/closure item for remaining unvisited/high-risk areas.\n"
            "- LATE PHASE (e.g., nodes coverage > ~70% or mature hypothesis set):\n"
            "  • Focus DEPTH: prioritize contradictions, deep dives on algorithms, hypothesis refinement/confirmation, and high-impact suspicions.\n"
            "  • ITEM GRANULARITY: Zoom into SMALLER, CONCRETE issues. Propose narrowly scoped, testable suspicions with exact node IDs and specific functions/storage to inspect.\n"
            "  • Still avoid over-concentration: do not allocate the whole batch to a single contract/aspect.\n\n"
            "INVESTIGATION STRATEGY:\n"
            "- Start with HIGH-LEVEL security aspects and broad architectural patterns; become progressively more specific as understanding accumulates.\n"
            "- Use coverage metrics and completed investigations to gauge audit maturity and adjust the Early/Mid/Late mix.\n"
            "- Early investigations should establish system understanding; later investigations should target specific vulnerabilities based on evidence.\n\n"
        ...
        user = (
            ...
            f"Apply the DIVERSITY CONSTRAINTS from the system instructions (distinct contracts/modules, aspects, and graphs; include an UNVISITED coverage item when appropriate).\n"
